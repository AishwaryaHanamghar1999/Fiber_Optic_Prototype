// ===== RECORDS EDIT SYSTEM =====
    ...
    function editRecord(tab, id) {
      let key = 'ofp_' + tab.replace('-', '_');
      if (tab === 'maintenance') key = 'ofp_complaints';
      const db = JSON.parse(localStorage.getItem(key) || '[]');
      const record = db.find(item => item.id === id);
      if (!record) return;

      window.editingRecord = { tab, id };
      const formId = tab === 'projects' ? 'projectWizardForm' : (tab === 'inventory' ? 'inventoryWizardForm' : (tab === 'outward-inventory' ? 'outwardInventoryWizardForm' : 'maintenanceWizardForm'));
    ...
    function persistAppState(panelId = currentPanel, recordsTab = activeRecordsTab) {
      localStorage.setItem('ofp_active_panel', panelId);
      localStorage.setItem('ofp_active_records_tab', recordsTab);
      if (currentUser) {
    ...
    function navigateTo(panelId, overrideMenuId = null) {
      ...
      persistAppState(panelId, activeRecordsTab);
    ...
    function updateClock() {
      ...
      if (clockEl) {
        clockEl.innerHTML = `<i class="fa-regular fa-clock"></i> ${now.toLocaleDateString('en-US', options)}`;
      }
    }
    ...
    function handleProjectSubmit(e) {
      ...
      if (window.editingRecord && window.editingRecord.tab === 'projects') {
        data.id = window.editingRecord.id;
        const idx = db.findIndex(item => item.id === data.id);
        if (idx !== -1) {
          data.dateCreated = db[idx].dateCreated;
          db[idx] = data;
        } else {
          db.unshift(data);
        }
        window.editingRecord = null;
        alert('✨ Project Sheet successfully updated!');
      } else {
        data.id = "P-" + Date.now();
    ...
    function handleInventorySubmit(e) {
      ...
      if (window.editingRecord && window.editingRecord.tab === 'inventory') {
        data.id = window.editingRecord.id;
        const idx = db.findIndex(item => item.id === data.id);
        if (idx !== -1) {
          data.dateCreated = db[idx].dateCreated;
        ...
        window.editingRecord = null;
        alert('✨ Material Receipt Record updated successfully!');
      } else {
        data.id = "I-" + Date.now();
        data.dateCreated = new Date().toISOString().split('T')[0];
        db.unshift(data);
    ...
    function handleOutwardInventorySubmit(e) {
      e.preventDefault();
      const form = e.target;
      const formData = new FormData(form);
      const data = Object.fromEntries(formData.entries());

      const db = JSON.parse(localStorage.getItem('ofp_outward_inventory') || '[]');

      if (window.editingRecord && window.editingRecord.tab === 'outward-inventory') {
        data.id = window.editingRecord.id;
        const idx = db.findIndex(item => item.id === data.id);
        if (idx !== -1) {
          data.dateCreated = db[idx].dateCreated;
        ...
        window.editingRecord = null;
        alert('✨ Outward Inventory Record updated successfully!');
      } else {
        data.id = "OI-" + Date.now();
        data.dateCreated = new Date().toISOString().split('T')[0];
        db.unshift(data);
        alert('✨ Outward Inventory Record saved successfully!');
      }

      localStorage.setItem('ofp_outward_inventory', JSON.stringify(db));
      form.reset();
      initWizardState('outward-inventory');
      navigateTo('records');
    }
    ...
    function handleMaintenanceSubmit(e) {
      ...
      if (window.editingRecord && window.editingRecord.tab === 'maintenance') {
        data.id = window.editingRecord.id;
        const idx = db.findIndex(item => item.id === data.id);
        if (idx !== -1) {
          data.dateCreated = db[idx].dateCreated;
        ...
        window.editingRecord = null;
        alert('✨ NOC Maintenance Ticket successfully updated!');
      } else {
        data.id = "M-" + Date.now();
        data.dateCreated = new Date().toISOString().split('T')[0];
        db.unshift(data);
    ...
    function switchRecordsTab(tab) {
      ...
      persistAppState(currentPanel, tab);
      ...
      if (activeRecordsTab === 'projects') {
          ...
          tr.innerHTML = `
            ...
            <td style="text-align: right;">
              <div class="action-btn-group" style="justify-content: flex-end;">
                ...
                <button class="action-btn" title="Edit record" onclick="editRecord('projects', '${item.id}')"><i class="fa-solid fa-pencil"></i></button>
          ...
          tr.innerHTML = `
            ...
            <td style="text-align: right;">
              <div class="action-btn-group" style="justify-content: flex-end;">
                ...
                <button class="action-btn" title="Edit record" onclick="editRecord('inventory', '${item.id}')"><i class="fa-solid fa-pencil"></i></button>
        ...
        head.innerHTML = `
          <tr>
            ...
            <th class="mobile-hide">Team/Vendor</th>
            <th class="mobile-hide">Material</th>
            <th class="mobile-hide">Quantity</th>
            <th>Issued By</th>
            <th style="width: 100px; text-align: right;">Actions</th>
          </tr>
        ...
        data.forEach(item => {
          ...
          tr.innerHTML = `
            ...
            <td><strong>${item.outwardDate || 'N/A'}</strong></td>
            <td class="mobile-hide">${item.location || 'N/A'}</td>
            <td class="mobile-hide">${item.evisionsTeamName || item.vendorName || 'N/A'}</td>
            <td class="mobile-hide"><span class="badge badge-info">${item.materialType || 'N/A'}</span></td>
            <td class="mobile-hide">${item.quantityTaken || '0'}</td>
            ...
            <td style="text-align: right;">
              <div class="action-btn-group" style="justify-content: flex-end;">
                ...
                <button class="action-btn" title="Edit record" onclick="editRecord('outward-inventory', '${item.id}')"><i class="fa-solid fa-pencil"></i></button>
          ...
          tr.innerHTML = `
            ...
            <td style="text-align: right;">
              <div class="action-btn-group" style="justify-content: flex-end;">
                ...
                <button class="action-btn" title="Edit record" onclick="editRecord('maintenance', '${item.id}')"><i class="fa-solid fa-pencil"></i></button>
    ...
    function showRecordDetails(tab, id) {
      ...
      const keyLabels = {
        ...
        evisionsTeamName: "Evisions Team Name",
        vendorName: "Vendor Name",
        fptName: "Fiber Pulling Team (FPT)",
        personTaken: "Material Taken By",
        materialTakenBy: "Material Taken By",
        issuedBy: "Material Issued By",
        quantityTaken: "Quantity Issued",
    ...
    function renderDashboardData() {
      // 1. Load Data
      const projects = readStoredData('ofp_projects', defaultMockProjects);
      const inventory = readStoredData('ofp_inventory', defaultMockInventory);
      const complaints = readStoredData('ofp_complaints', defaultMockComplaints);
      ...
      // 4.1 Render Projects by Type
      const projTypes = {};
      projects.forEach(p => {
        const type = p.projectType || 'Other';
        projTypes[type] = (projTypes[type] || 0) + 1;
      });
      const projContainer = document.getElementById('projects-pie-chart');
      const projLegends = document.getElementById('projects-pie-legends');
      if (projContainer && projLegends) {
        projContainer.innerHTML = '';
        projLegends.innerHTML = '';
        const maxProj = Math.max(...Object.values(projTypes), 1);
        const colors = ['var(--primary)', 'var(--accent)', 'var(--info)', 'var(--warning)'];
        Object.keys(projTypes).forEach((type, idx) => {
          const count = projTypes[type];
          const pct = Math.max((count / maxProj) * 100, 10);
          const color = colors[idx % colors.length];
          // bar
          projContainer.innerHTML += `<div style="width: 40px; height: ${pct}%; background: ${color}; border-radius: 4px 4px 0 0; position: relative; display: flex; flex-direction: column; justify-content: flex-end; align-items: center;" title="${type}"><span style="position: absolute; top: -20px; font-size: 12px; font-weight: bold;">${count}</span></div>`;
          // legend
          projLegends.innerHTML += `<div class="legend-item"><div class="legend-label"><span class="legend-dot" style="background:${color}"></span> ${type}</div></div>`;
        });
      }

      // 4.2 Render Material Distribution
      const matTypes = {};
      inventory.forEach(i => {
        const type = i.materialType || 'Other';
        matTypes[type] = (matTypes[type] || 0) + (parseFloat(i.quantity) || 0);
      });
      const matContainer = document.getElementById('materials-bar-chart');
      if (matContainer) {
        matContainer.innerHTML = '';
        const sortedMats = Object.keys(matTypes).sort((a,b) => matTypes[b] - matTypes[a]).slice(0, 3);
        const maxMat = Math.max(...sortedMats.map(k => matTypes[k]), 1);
        sortedMats.forEach(type => {
          const qty = matTypes[type];
          const pct = Math.max((qty / maxMat) * 100, 5);
          matContainer.innerHTML += `
            <div style="display: flex; align-items: center; gap: 1rem; width: 100%;">
              <div style="width: 100px; font-size: 0.75rem; font-weight: 600; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;" title="${type}">${type}</div>
              <div style="flex-grow: 1; background: var(--bg-app); height: 24px; border-radius: 12px; overflow: hidden; position: relative;">
                <div style="width: ${pct}%; background: var(--accent); height: 100%; border-radius: 12px;"></div>
                <span style="position: absolute; right: 8px; top: 3px; font-size: 0.75rem; color: ${pct > 80 ? 'white' : 'var(--text-main)'}; font-weight: 600;">${qty}</span>
              </div>
            </div>`;
        });
      }
      ...
      if (limit.length === 0) {
        ...
        limit.forEach(item => {
          ...
          tr.innerHTML = `
            <td>${item.date}</td>
            <td class="mobile-hide"><strong>${item.category}</strong></td>
            <td><code>${item.identifier}</code></td>
            <td class="mobile-hide">${item.actor}</td>
    ...
    function populateBillingWO() {
      const select = document.getElementById('billingSearchWO');
      if (!select) return;
      ...
      projects.forEach(p => {
        if (p.workOrder) {
          ...
          opt.value = p.workOrder;
          opt.textContent = p.workOrder + ' (' + (p.city || 'Unknown') + ')';
    ...
    function loadBillingForWO() {
      ...
      const btnAdd = document.getElementById('btnAddNewBillTop');
      const container = document.getElementById('billingListContainer');
    ...
    function renderBillingTable() {
      ...
      const emptyState = document.getElementById('billingTableEmpty');
      const tableWrapper = document.getElementById('billingTableWrapper');
      const formWrapper = document.getElementById('billingFormWrapper');
      ...
      currentBills.forEach((bill, index) => {
        ...
        tr.innerHTML = `
          ...
          <td class="mobile-hide">${bill.poNo || '-'}</td>
          <td>${bill.invoiceNo || '-'}</td>
          <td class="mobile-hide">${bill.baseAmount ? '₹' + bill.baseAmount : '-'}</td>
          <td>${bill.finalAmount ? '₹' + bill.finalAmount : '-'}</td>
          <td class="mobile-hide"><span class="badge ${bill.status === 'Done' ? 'badge-success' : 'badge-warning'}">${bill.status || '-'}</span></td>
          <td class="mobile-hide"><span class="badge ${bill.paymentStatus === 'Fully Received' ? 'badge-success' : (bill.paymentStatus === 'Overdue' ? 'badge-danger' : 'badge-warning')}">${bill.paymentStatus || '-'}</span></td>
          <td class="text-right">
            <div class="action-btn-group">
              <button class="action-btn" title="Edit Bill" onclick="showBillingForm(${index})"><i class="fa-solid fa-pencil"></i></button>
              <button class="action-btn delete-btn" title="Delete Bill" onclick="removeBillingCard(${index})"><i class="fa-solid fa-trash-can"></i></button>
            </div>
          </td>
        `;
    ...
    function showBillingForm(index) {
      ...
      let bill = {};
      if (index >= 0) {
        bill = currentBills[index];
      } else {
        bill = {
          status: 'Done', poDate: '', poNo: '', invoiceDate: '', invoiceNo: '',
          invoiceSent: 'Yes', baseAmount: '', gst: '', finalAmount: '', arn: '',
          approver: '', paymentDueDate: '', paymentStatus: 'Not Received', finalRemark: ''
        };
      }
      ...
      formWrapper.innerHTML = `
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1.5rem; border-bottom: 1px solid var(--border-color); padding-bottom: 0.75rem;">
          <h4 style="font-weight: 600; color: #1e293b; font-size: 1.1rem; display: flex; align-items: center; gap: 0.6rem;">
            <div style="background:var(--accent); color:white; width: 28px; height: 28px; display:flex; align-items:center; justify-content:center; border-radius:6px; font-size:0.8rem;">
              <i class="fa-solid fa-file-invoice-dollar"></i>
            </div>
            ${index >= 0 ? 'Edit Bill #' + (index + 1) : 'Add New Bill'}
          </h4>
        </div>
        <form id="billingForm" onsubmit="saveSingleBill(event)">
          <div class="form-grid">
            <div class="field-group"><label><i class="fa-solid fa-circle-check"></i> BILLING STATUS</label>
              <select name="status">
            ...
            <div class="field-group"><label><i class="fa-regular fa-calendar-days"></i> PO DATE</label><input type="date" name="poDate" value="${bill.poDate || ''}"></div>
            <div class="field-group"><label><i class="fa-solid fa-file-invoice"></i> PO NO.</label><input type="text" name="poNo" value="${bill.poNo || ''}"></div>
            <div class="field-group"><label><i class="fa-regular fa-calendar-days"></i> INVOICE DATE</label><input type="date" name="invoiceDate" value="${bill.invoiceDate || ''}"></div>
            <div class="field-group"><label><i class="fa-solid fa-hashtag"></i> INVOICE NO.</label><input type="text" name="invoiceNo" value="${bill.invoiceNo || ''}"></div>
            <div class="field-group"><label><i class="fa-solid fa-paper-plane"></i> INVOICE SENT</label>
              <select name="invoiceSent">
                <option value="Yes" ${bill.invoiceSent === 'Yes' ? 'selected' : ''}>Yes</option>
                <option value="No" ${bill.invoiceSent === 'No' ? 'selected' : ''}>No</option>
              </select>
            </div>
            <div class="field-group"><label><i class="fa-solid fa-coins"></i> BASE AMOUNT</label><input type="number" name="baseAmount" placeholder="0.00" value="${bill.baseAmount || ''}"></div>
            <div class="field-group"><label><i class="fa-solid fa-percent"></i> GST</label><input type="number" name="gst" placeholder="0.00" value="${bill.gst || ''}"></div>
            <div class="field-group"><label><i class="fa-solid fa-money-bill-wave"></i> FINAL AMOUNT</label><input type="number" name="finalAmount" placeholder="0.00" value="${bill.finalAmount || ''}"></div>
            <div class="field-group"><label><i class="fa-solid fa-hashtag"></i> ARN</label><input type="text" name="arn" value="${bill.arn || ''}"></div>
            <div class="field-group"><label><i class="fa-solid fa-user-check"></i> APPROVER</label><input type="text" name="approver" value="${bill.approver || ''}"></div>
            <div class="field-group"><label><i class="fa-regular fa-calendar-days"></i> PAYMENT DUE</label><input type="date" name="paymentDueDate" value="${bill.paymentDueDate || ''}"></div>
            <div class="field-group"><label><i class="fa-regular fa-credit-card"></i> PAYMENT STATUS</label>
            ...
            <div class="field-group"><label><i class="fa-solid fa-comment-dots"></i> FINAL REMARK</label>
              <textarea style="width:100%; border:1px solid var(--border-color); border-radius:8px; padding:0.75rem; outline:none; font-family:inherit; min-height: 48px; resize: vertical;" name="finalRemark">${bill.finalRemark || ''}</textarea>
            </div>
          </div>
          <div style="display: flex; gap: 1rem; margin-top: 2rem; justify-content: flex-end;">
            <button type="button" class="btn btn-secondary" onclick="renderBillingTable()">Cancel</button>
            <button type="submit" class="btn btn-success"><i class="fas fa-check"></i> ${index >= 0 ? 'Update Bill' : 'Add Bill'}</button>
          </div>
        </form>
      `;
    }

    function saveSingleBill(e) {
      e.preventDefault();
      const form = e.target;
      const formData = new FormData(form);
      const data = Object.fromEntries(formData.entries());
      ...
      if (editingBillIndex >= 0) {
        currentBills[editingBillIndex] = data;
      } else {
        currentBills.push(data);
      }
